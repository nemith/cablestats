...
class ComcastBCModem(object):
    ...
    # 5 Minutes (in secs). This is a total guess and can probably be increased
    # depening on the actual timeout
    ...
    def login(self, username=None, password=None):
        ...
        # We are very stupidly assuming we are logged in here.  There should be
        # a check for the validity of the cookie
            ...
            for line in script_element.splitlines():
                ...
                for match in re.finditer('var\s*(?P<var>\S+)\s*=\s*(?P<value>[^$;]+)', line):
                    ...
                    if var == "cable_status":
                        ...
                        m = re.match(
                            'var Cm(?P<direction>Downstream|Upstream)(?P<attr>\S+)Base\s*=\s*"(?P<value>[^"]+)"', line)
                        if not m:
                            continue
                        values = [v.strip()
                                  for v in m.group('value').split('|') if v]
                        for idx, value in enumerate(values):
                            key = camel_to_under(m.group('attr'))
                            channel = "ch" + str(idx)

                            if m.group('direction') == 'Downstream':
                                ch_data = cm_ds_channels.setdefault(
                                    channel, {})
                            elif m.group('direction') == 'Upstream':
                                ch_data = cm_us_channels.setdefault(
                                    channel, {})
